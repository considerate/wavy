...
import Data.Binary.Get hiding(getInt8, getInt16le, getInt32le, getInt64le)
import Data.Binary.Put hiding(putInt8, putInt16le, putInt32le, putInt64le)
import Data.Bits
...
encodeIntegralWaveData file rawData = file { waveData = runPut putter }
  where putter = putIntegralWaveData (waveFormat file) rawData
...
encodeFloatingWaveData file rawData = file { waveData = runPut putter }
  where putter = putFloatingWaveData (waveFormat file) rawData
...
putIntegralWaveData format (IntegralWaveData rawData) = mapM_ putter
  $ interleaveData rawData
  where
    putter :: Int64 -> Put
...
putFloatingWaveData
  ...
  -> Put               -- ^ A Put context that will be used to write the data out.
putFloatingWaveData format (FloatingWaveData rawData) = mapM_ (putter . floatToInt)
  $ interleaveData rawData
  where
    putter :: Int64 -> Put
    putter = wordPutter (bytesPerChannelSample format)
